...
if uploaded_files:
    try:
        ...
        for uploaded_file in uploaded_files:
            try:
                # StringIOを使わずに直接アップロードされたファイルオブジェクトを渡す
                df_single = pd.read_csv(uploaded_file, encoding='shift_jis')
            except UnicodeDecodeError:
                uploaded_file.seek(0) # seek(0)でポインタを先頭に戻す
                df_single = pd.read_csv(uploaded_file, encoding='utf-8')
        ...
        if 'user' not in df.columns or 'message' not in df.columns:
            ...
            st.success(f'{len(uploaded_files)}個のファイルの読み込みに成功しました！')
