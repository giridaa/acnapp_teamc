...
def evaluate_work_environment(df):
    ...
    # 2. 生データの計算
    daily_summary['Daily_Overtime'] = (
        daily_summary['Total Work Duration'] - pd.Timedelta(hours=8)
    )
    daily_summary['Daily_Overtime'] = daily_summary['Daily_Overtime'].apply(
        lambda x: max(x, pd.Timedelta(0))
    )

    high_daily_overtime_days = daily_summary[
        daily_summary['Daily_Overtime'] >= pd.Timedelta(hours=5)
    ]
    ...
    if not high_daily_overtime_days.empty:
        ...
        high_daily_overtime_days['Date'] = pd.to_datetime(
            high_daily_overtime_days['Date']
        )
        monthly_high_ot_counts = high_daily_overtime_days.groupby(
            high_daily_overtime_days['Date'].dt.to_period('M')
        ).size()
        if (monthly_high_ot_counts >= 4).any():
            is_dangerous_by_daily_ot = True

    overtime_days_full = daily_summary[
        daily_summary['Total Work Duration'] > pd.Timedelta(hours=8)
    ]
    ...
    weekly_work_hours = daily_summary.groupby([
        daily_summary['Date'].dt.isocalendar().year,
        daily_summary['Date'].dt.isocalendar().week
    ])['Total Work Duration'].sum().reset_index()
    weekly_work_hours.columns = ['Year', 'Week', 'Total Weekly Duration']
    weekly_work_hours['Overtime'] = (
        weekly_work_hours['Total Weekly Duration'] - pd.Timedelta(hours=40)
    )
    weekly_work_hours['Overtime'] = weekly_work_hours['Overtime'].apply(
        lambda x: max(x, pd.Timedelta(0))
    )

    def get_week_start_date(row):
        return pd.to_datetime(
            f'{int(row["Year"])}-W{int(row["Week"])}-1', format='%G-W%V-%u'
        )
    weekly_work_hours['Week_Start_Date'] = weekly_work_hours.apply(
        get_week_start_date, axis=1
    )

    average_weekly_overtime = weekly_work_hours['Overtime'].mean()
    avg_overtime_hours = (
        average_weekly_overtime.total_seconds() / 3600
        if pd.notna(average_weekly_overtime) else 0
    )

    dangerous_weeks = weekly_work_hours[
        weekly_work_hours['Overtime'] >= pd.Timedelta(hours=10)
    ]
    is_dangerous_by_weekly_ot = not dangerous_weeks.empty
    is_dangerous = is_dangerous_by_weekly_ot or is_dangerous_by_daily_ot

    inadequate_break_days = overtime_days_full[
        overtime_days_full['Total Break Duration'] < pd.Timedelta(hours=1)
    ]
    ...
    if is_dangerous:
        if is_dangerous_by_weekly_ot:
            display_results["残業が多い週(週残業10h超)"] = ", ".join(
                dangerous_weeks['Week_Start_Date'].dt.strftime(
                    '%Y/%m/%d週'
                ).tolist()
            )
        if is_dangerous_by_daily_ot:
...
def generate_chart_data(all_dfs):
    ...
    for df in all_dfs:
        ...
        weekly_summary = daily_summary.groupby([
            ...
            OT_Days=('Total Work Duration',
                     lambda x: (x > pd.Timedelta(hours=8)).sum())
        ).reset_index()
        weekly_summary.columns = ['Year', 'Week', 'Total Weekly Duration',
                                  'OT_Count']

        weekly_summary['Overtime'] = (
            weekly_summary['Total Weekly Duration'] - pd.Timedelta(hours=40)
        )
        weekly_summary['Overtime'] = weekly_summary['Overtime'].apply(
            lambda x: max(x, pd.Timedelta(0))
        )
    ...
    project_avg_weekly['Week_Start_Date'] = project_avg_weekly.apply(
        get_week_start_date, axis=1
    )
    ...
    full_date_range = pd.date_range(
        start=six_months_ago, end=latest_date, freq='W-MON'
    )

    project_avg_weekly = project_avg_weekly.reindex(
        full_date_range, fill_value=0
    )

    if 'Overtime' in project_avg_weekly.columns:
        project_avg_weekly['Overtime'] = pd.to_timedelta(
            project_avg_weekly['Overtime']
        )
    ...
    chart_df = pd.DataFrame({
        '週平均残業時間(h)': (
            project_avg_weekly['Overtime'].dt.total_seconds() / 3600
        ),
...
def create_multi_user_radar_chart(result_df, my_name):
    ...
    for index, row in result_df.iterrows():
        ...
        fig.add_trace(go.Scatterpolar(
            ...
            hovertemplate=(
                f'<b>{user_name}</b><br>%{{theta}}: %{{r}}<extra></extra>'
            )
        ))
...
def generate_persona_with_retry(
    target_user_name, target_user_scores, target_user_text,
    my_scores, max_retries=3
):
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            # レスポンスがクリーンなJSONであることを期待して、クリーニング処理を簡素化
            ...
            if all(k in persona_data for k in [
                "persona", "common_point", "communication_point"
            ]):
                return persona_data
        except (json.JSONDecodeError, Exception) as e:
            st.error(
                f"AIからの応答処理中にエラー（試行 {attempt + 1}回目）: {e}"
            )
...
def generate_team_atmosphere(text, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if all(k in atmosphere_data for k in [
                "atmosphere", "description", "weather"
            ]):
                return atmosphere_data
        except (json.JSONDecodeError, Exception) as e:
            st.error(
                f"チーム雰囲気のAI分析中にエラー（試行 {attempt + 1}回目）: {e}"
            )
            if attempt == max_retries - 1:
                return default_response
    return default_response
...
def generate_overall_evaluation(
    atmosphere_result, result_df, work_analysis_result, my_name, max_retries=3
):
    """
    ...
    my_dominant_personality = (
        my_data['最も強い性格傾向'].iloc[0] if not my_data.empty else '不明'
    )

    other_members_data = result_df[result_df['ユーザー'] != my_name]
    average_match_score = (
        other_members_data['自分との性格マッチ度 (%)'].mean()
        if not other_members_data.empty else 0
    )
    team_composition = (
        other_members_data['最も強い性格傾向'].value_counts().to_dict()
    )
    ...
    以下の多角的な分析結果を基に、ユーザー（{my_name}さん）が
    このプロジェクトに参加すべきかどうか、総合的な評価とアドバイスをしてください。
    ...
    - **必ず、以下のキーを持つJSONオブジェクト"だけ"を生成してください。**
    - **解説や前置き、```jsonのような追加の文字列は絶対に含めないでください。**
    - **"recommendation"の値は、必ず以下のリストから
      最も適切だと思うものを1つだけ選んでください。**
      {recommendation_options}
    - **"reason"は、その結論に至った根拠を30文字程度で具体的に述べてください。**

    {{
      "recommendation": "（上記のリストから選択）",
      "reason": "（評価理由を40文字程度で説明）"
    }}
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            st.error(
                f"総合評価のAI分析中にエラー（試行 {attempt + 1}回目）: {e}"
            )
            if attempt == max_retries - 1:
                return default_response
...
def get_recommendation_color(recommendation_str):
    """ 推奨度に応じて色を返す """
    # 絵文字や感嘆符を除いた文字列で判定する
    if "強く推奨する" in recommendation_str:
        return "green"
    elif "推奨する" in recommendation_str:
        return "blue"
    elif "推奨しない" in recommendation_str:
        return "red"
    else:  # 自己判断に委ねる
        return "orange"
...
st.write(
    'アサイン予定のPJメンバーのチャットデータ、MTG会話データ、勤怠データと'
    'あなたのチャットデータ(CSV)をアップロードすると、チームの雰囲気や労働環境、'
    'メンバーの性格傾向を分析し、PJとあなたのマッチングを診断します。'
)
st.write('---')
...
with col2:
    ...
    transcript_files = st.file_uploader(
        "音声テキストCSVを選択", type="csv",
        accept_multiple_files=True, key="transcript_uploader"
    )
with col3:
    st.subheader("🗨 自分のチャット")
    my_file = st.file_uploader(
        "自分のチャットCSVを選択", type="csv",
        accept_multiple_files=False, key="mychat_uploader"
    )
with col4:
    st.subheader("📆 PJの勤怠データ")
    work_files = st.file_uploader(
        "勤怠データCSVを選択", type="csv",
        accept_multiple_files=True, key="work_uploader"
    )
st.write('---')
...
if (chat_files or transcript_files or work_files) and my_file:
    try:
        ...
        team_chat_df = (
            pd.concat(team_chat_dfs, ignore_index=True)
            if team_chat_dfs else pd.DataFrame()
        )
        ...
        if transcript_files:
            ...
            for file in transcript_files:
                file.seek(0)
                try:
                    # header=None を追加し、1行目をヘッダーとして扱わない
                    df_single = pd.read_csv(
                        file, header=None, encoding='shift_jis'
                    )
                except UnicodeDecodeError:
                    file.seek(0)
                    # こちらにも header=None を追加
                    df_single = pd.read_csv(
                        file, header=None, encoding='utf-8'
                    )
                team_transcript_dfs.append(df_single)

            if team_transcript_dfs:
                team_transcript_df = pd.concat(
                    team_transcript_dfs, ignore_index=True
                )
                # 'message'列ではなく、0番目（最初）の列を指定してテキストを抽出
                if not team_transcript_df.empty and (
                    0 in team_transcript_df.columns
                ):
                    transcript_text = ' '.join(
                        team_transcript_df[0].fillna('').astype(str)
                    )
        ...
        if work_files:
            for file in work_files:
                ...
                try:
                    ...
                    file.seek(0)
                    df_single = pd.read_csv(file, encoding='utf-8')
                # ユーザー名をファイル名から取得（仮）CSVに'user'列があれば優先
        ...
        st.success(
            f'{len(chat_files) + len(transcript_files) + len(work_files) + 1}'
            '個のファイルの読み込みに成功！'
        )
        ...
        if st.button('分析を実行する'):
            st.write('---')
            st.header('分析結果')
            atmosphere_result, result_df = None, pd.DataFrame()  # 結果を初期化
            ...
            if transcript_text:
                ...
                with st.spinner('AIがチームの雰囲気を分析中です...'):
                    atmosphere_result = generate_team_atmosphere(
                        transcript_text
                    )
                    ...
                    col_atm1.metric(
                        "現在のチームの天気",
                        weather_str, get_weather_icon(weather_str)
                    )
                    col_atm2.info(
                        f"**雰囲気**: {atmosphere_result.get('atmosphere', 'N/A')}"
                    )
                    col_atm3.success(
                        f"**理由**: {atmosphere_result.get('description', 'N/A')}"
                    )
                st.write('---')
            ...
            if not df.empty and 'user' in df.columns and df['user'].nunique() > 1:
                ...
                user_texts = df.groupby('user')['message'].agg(
                    ' '.join
                ).reset_index()
                results = []
                with st.spinner('性格スコアを分析中です...'):
                    for _, row in user_texts.iterrows():
                        ...
                        results.append({
                            'ユーザー': row['user'],
                            '最も強い性格傾向': get_dominant_personality(scores),
                ...
                if my_name in result_df['ユーザー'].values:
                    my_scores_row = result_df[
                        result_df['ユーザー'] == my_name
                    ].iloc[0]
                    my_scores = my_scores_row['スコア詳細']
                    my_scores_list = list(my_scores.values())
                    match_percentages = [
                        round(calculate_cosine_similarity(
                            my_scores_list, list(row['スコア詳細'].values())
                        ) * 100, 2)
                    ...
                    st.subheader('🎭 2.PJメンバーのペルソナ分析')
                    other_users_df = result_df[
                        result_df['ユーザー'] != my_name
                    ...
                    with st.spinner('AIが各メンバーのペルソナを生成中です...'):
                        ...
                        for i in range(0, len(user_list), 2):
                            ...
                            _, row1 = user_list[i]
                            with col1.expander(
                                f"**{row1['ユーザー']}さん** のペルソナを見る",
                                expanded=True
                            ):
                                persona = generate_persona_with_retry(
                                    row1['ユーザー'], row1['スコア詳細'],
                                    row1['全発言'], my_scores
                                )
                                st.info(
                                    f"**人柄**: {persona.get('persona', 'N/A')}"
                                )
                                st.success(
                                    f"**共通点**: "
                                    f"{persona.get('common_point', 'N/A')}"
                                )
                                st.warning(
                                    f"**ポイント**: "
                                    f"{persona.get('communication_point', 'N/A')}"
                                )
                            # 2人目 (存在すれば)
                            if (i + 1) < len(user_list):
                                _, row2 = user_list[i + 1]
                                with col2.expander(
                                    f"**{row2['ユーザー']}さん** のペルソナを見る",
                                    expanded=True
                                ):
                                    persona = generate_persona_with_retry(
                                        row2['ユーザー'], row2['スコア詳細'],
                                        row2['全発言'], my_scores
                                    )
                                    st.info(
                                        f"**人柄**: "
                                        f"{persona.get('persona', 'N/A')}"
                                    )
                                    st.success(
                                        f"**共通点**: "
                                        f"{persona.get('common_point', 'N/A')}"
                    ...
                    st.write('---')
                    st.subheader(f'🫡 3.{my_name} とPJメンバーの性格マッチ度')
                    ...
                    st.write('---')
                    st.subheader('📈 4.自分とPJメンバーの性格比較チャート')
                    st.plotly_chart(
                        create_multi_user_radar_chart(result_df, my_name),
                        use_container_width=True
                    )

                else:
                    st.info(
                        "性格分析を行うには、PJメンバーのチャットファイルと"
                        "自分のチャットファイルを両方アップロードしてください。"
                    )
            ...
            if all_member_work_dfs:
                ...
                with st.spinner('勤怠データを分析中です...'):
                    individual_results = []
                    # ユーザーごとに勤怠データを結合して分析
                    all_work_df_combined = pd.concat(
                        all_member_work_dfs, ignore_index=True
                    )
                    unique_users = all_work_df_combined['user'].unique()

                    user_dfs_for_chart = []
                    for user in unique_users:
                        user_df = all_work_df_combined[
                            all_work_df_combined['user'] == user
                        ].copy()
                        user_dfs_for_chart.append(user_df)
                        display_res, raw_res = evaluate_work_environment(
                            user_df
                        )
                    ...
                    if num_individuals > 0:
                        avg_weekly_ot = sum(
                            res['raw']['avg_overtime_hours']
                            for res in individual_results
                        ) / num_individuals
                        
                        project_overtime_trend = "通常"
                        project_trend_reason = "プロジェクト全体で基準の範囲内です"
                        is_project_dangerous = any(
                            res['raw']['is_dangerous']
                            for res in individual_results
                        )

                        if is_project_dangerous:
                            project_overtime_trend = "危険な労働環境"
                            project_trend_reason = (
                                "個人評価に「過重労働傾向あり」のメンバーが含まれています"
                            )
                        elif avg_weekly_ot >= 5:
                            project_overtime_trend = "残業傾向あり"
                            project_trend_reason = (
                                "プロジェクトの週平均残業時間が5時間を超えています"
                            )

                        st.info(
                            f"**プロジェクト全体の残業評価: {project_overtime_trend}** "
                            f"({project_trend_reason})"
                        )

                        # 勤怠分析の結果を総合評価用に保存
                        work_analysis_result = {
                            'trend': project_overtime_trend,
                            'reason': project_trend_reason
                        }
            ...
            if atmosphere_result and not result_df.empty:
                with st.spinner('AIがすべての結果を統合し、最終評価を生成中です...'):
                    ...
                    evaluation = generate_overall_evaluation(
                        atmosphere_result, result_df,
                        work_analysis_result, my_name
                    )
                    ...
                    st.markdown(
                        f"### あなたへの推奨度: <font color='{color}'>"
                        f"**{recommendation}**</font>",
                        unsafe_allow_html=True
                    )
                    st.info(f"**理由**: {reason}")
            else:
                st.warning(
                    "総合評価を行うには、PJメンバーの「MTG会話」と"
                    "「Teamsチャット」の両方のデータをアップロードして"
                    "分析を実行する必要があります。"
                )
